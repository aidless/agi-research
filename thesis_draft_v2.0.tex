...
satisfies \texttt{lim/\\1{} [[]] --?1} if and only if  is provably true in classical
logic.
...
If each physics module \texttt{M/\\1i} is correct on its support set \texttt{S/\\1i}, then the
Composer \texttt{C} learns the correct module-mixing weights with sample complexity
\texttt{O(/textbar{}S\textbar{}\^{}2 log(1/))} where $S = ?S/^{1}i$.
...
with \texttt{P/K} parameters per module achieves worst-case error $O((P/K)///^{${}{}{?/d} / )}.
...
with \texttt{O(T log(1/))} samples where \texttt{T} is horizon, beating PPO''s $O(T///^{${}{}2 / )}
sample complexity for \texttt{ < 1/T}.

\paragraph{Theorem 11 --?Active Inference Convergence}

Under regularity conditions, the expected free energy \texttt{G/^{1}t} decreases
monotonically with iteration \texttt{t}, and \texttt{lim/^{1}{t--} G\_t = G*}.
...
\end{itemize}
  ...
  fire right).
\begin{itemize}
  \item \texttt{P(s''/textbar{}s, a)}: deterministic transition function from the Box2D physics engine.
...
that maps an episode history $h/\\1t = (s/\\10, a/\\10, r/\\10, --? s/\\1t)$ to a predicted
...
\textbf{H1}: A Monitor trained on rollouts from a frozen policy \texttt{--/^{1}f} has higher
...
\begin{itemize}
  \item Frozen Monitor: PPO trains a policy \texttt{--/^{1}f} for 100K steps; the policy is then
\end{itemize}

  frozen; 5000 additional episodes are collected from \texttt{--/^{1}f}; the Monitor is
...
LunarLander-v3 from Gymnasium. We use \texttt{make/^{1}env(env/^{1}name, seed)} from \texttt{envs.py}
...
(those from \texttt{--/^{1}f}). The joint Monitor is trained on a non-stationary
distribution (those from \texttt{--/^{1}t}, which changes every PPO update). This covariate
...
2. \textit{Pseudo-failures} induced by the policy''s exploration: actions that are bad for
   the current policy but would have been good for \texttt{--/^{1}f}.
...
1. Projects input and slots to a shared \texttt{key}^{1}texttt{query}^{1}texttt{value} space.
...
In \texttt{slot/^{1}monitor.py}, the input history vector is reshaped into \texttt{(T, F)},
...
implementation is in \texttt{active/^{1}inference.py} and \texttt{aie/^{1}lunarlander.py}:

\begin{itemize}
  \item \textbf{Encoder} \texttt{q(s/textbar{}o)}: MLP producing posterior mean and log-variance over a
\end{itemize}

  latent state.
\begin{itemize}
  \item \textbf{Generation model} \texttt{p(o/textbar{}s)}: MLP decoding latent state to predicted
\end{itemize}

  observation.
\begin{itemize}
  \item \textbf{Transition model} \texttt{p(s''/textbar{}s, a)}: linear layer that takes the current state
\end{itemize}
...
\end{itemize}
  ...
  $F = E/\\1q[log q(s) ?log p(o, s)]$.
\begin{itemize}
  \item \textbf{Action sampler}: policy network mapping latent state to action logits.
  \item \textbf{Preference model} \texttt{p(o/textbar{}C)}: learnable parameter representing the goal in
\end{itemize}
...
\texttt{aie/^{1}lunarlander.py} with \texttt{n/^{1}episodes=30}, \texttt{n/^{1}epochs=15}. The result is recorded
in \texttt{experiments/^{1}log/2026-07-27-aie-training.md}.
...
The slot attention module is in \texttt{projects/project/^{1}c/^{1}causal/^{1}world/code/}.
Key parameters:

\begin{itemize}
  \item \texttt{slot/^{1}dim = 32} (per-slot feature dimension)
  \item \texttt{n/^{1}slots = 4} (matches LunarLander''s natural decomposition)
  \item \texttt{n/^{1}iters = 3} (default; 1 suffices for 1-D inputs)
...
A \textit{world model} predicts \texttt{p(s''/textbar{}s, a)}. With slot-structured states, we can learn
...
\texttt{f/^{1}k} sees only the dynamics relevant to its slot.

\subsubsection{11.2 Architecture}

The per-slot dynamics model in \texttt{slot/^{1}attention/^{1}dynamics.py}:
...
We port the four physics modules from \texttt{F:/TMLR/Fusion/enwi/^{1}prototype/}:

\begin{itemize}
  \item \textbf{Gravity}: $y'' = g ?drag * y'$. Implemented as a 2-layer MLP over \texttt{(y, v/^{1}y)}.
  \item \textbf{Collision}: \texttt{v' = -- /textit{ v + (1 + e) } m\_other / (m\_self + m\_other) * v\_other}.
\end{itemize}

  Implemented as an MLP over \texttt{(v/^{1}self, v/^{1}other, m/^{1}self, m/^{1}other)}.
\begin{itemize}
...
We replicate this in \texttt{enwi/^{1}prediction2.py}:
...
The language interface in \texttt{code/language/^{1}interface.py} uses a small template:
...
\begin{itemize}
  \item \textbf{Position}: $(x, y) ?R--$. Describes where the lander is.
  \item \textbf{Velocity}: $(vx, vy) ?R--$. Describes how the lander moves.
  \item \textbf{Rotation}: $(angle, ang/\\1vel) ?R  R$. Describes orientation.
  \item \textbf{Contact}: $(leg/\\1l, leg/\\1r) ?{0, 1$--}. Describes ground contact.
\end{itemize}
...
\begin{itemize}
  \item $Position + Velocity * --t --?Position$: kinematic integration.
  \item $Velocity --?Velocity$: thrust application.
  \item $Contact --?{landed, crashed$}: terminal state derivation.
\end{itemize}
...
\begin{itemize}
  \item \texttt{leg/^{1}contact(state)}: leg(s) touching ground.
  \item \texttt{velocity/^{1}below(state, threshold)}: ------?< threshold.
  \item \texttt{angle/^{1}below(state, threshold)}: |angle| < threshold.
  \item \texttt{landed(state)}: terminal state with reward ?100.
  \item \texttt{in/^{1}pad(state)}: position within landing pad.
  \item \texttt{distance/^{1}to/^{1}pad(state)}: ---- ?x\_pad--?
\end{itemize}
...
The verifier in \texttt{code/ltl/^{1}verifier.py}:
...
For binary predicates (e.g., \texttt{left/^{1}of(x, y)}), the network takes the
concatenation of two slot features.

\subsubsection{16.5 The LunarLander integration}

\texttt{dlr/^{1}lunarlander.py} instantiates four LunarLander predicates:

\begin{itemize}
  \item \texttt{landed}: terminal state with reward ?100.
  \item \texttt{upright}: |angle| < 0.3.
  \item \texttt{leg/^{1}l/^{1}contact}: leg\_l == 1.
  \item \texttt{leg/^{1}r/^{1}contact}: leg\_r == 1.
\end{itemize}
...
\begin{itemize}
  \item \texttt{monitor/^{1}prob} (1-dim)
  \item \texttt{slot/^{1}states} (4  32-dim = 128-dim)
  \item \texttt{recent/^{1}rewards} (10-dim)
\end{itemize}
...
\begin{itemize}
  \item Each agent \texttt{i} has its own Slot-Monitor \texttt{M/^{1}i}.
...
\texttt{papers/monitor/^{1}signal/^{1}vs/^{1}dlr/^{1}6pathway.{md,tex,pdf}}.
...
Our Procgen baseline in \texttt{procgen/^{1}baseline.py} is ready but unrun: Procgen
...
For the full Y5 paper, see \texttt{papers/y5/^{1}monitor/^{1}transfer/^{1}synthesis.md}.
...
For the full Y4 paper, see \texttt{papers/project/^{1}g/^{1}v0/^{1}5/^{1}h10/^{1}paper.md}.
...
\texttt{lim/^{1}{} [[]] --?1} iff  is provably true in classical logic.
...
Formally: for any formula \texttt{(x)} over base predicates \texttt{p/^{1}i}, let \texttt{[[]]/^{1}} be the
value computed by the DLR with parameters `\texttt{. As } --?*` (the limit of perfect
predicate approximation), we have $[[]]/\\1 --?[[]]/\\1classical ?{0, 1$}.
...
where \texttt{p/^{1}k} ranges over all task-relevant predicates. By construction,
\texttt{[[(o)]] = 1} iff \texttt{s} agrees with \texttt{E(o)} on every \texttt{p/^{1}k}, which is the
definition of faithful abstraction.

\subsubsection{A.3 Theorem 7 --?Composable Physics Sample Complexity}

\textbf{Statement}: If each physics module \texttt{M/^{1}i} is correct on its support set
\texttt{S/^{1}i}, then the Composer \texttt{C} learns the correct module-mixing weights with
sample complexity \texttt{O(/textbar{}S\textbar{}\^{}2 log(1/))} where $S = ?S/\\1i$.
...
\texttt{n = O(log(1/) / --)}. For  = 1/|S|, we have \texttt{n = O(/textbar{}S/textbar{}-- log(1/))}.
...
modules themselves. This reduces the effective complexity from \texttt{K * /textbar{}S/textbar{}} to
\texttt{/textbar{}S/textbar{}}.
...
\hline
\texttt{projects/project/\\1c/\\1causal/\\1world/code/composable/\\1physics.py} & \texttt{F:/TMLR/Fusion/enwi/\\1prototype/composable/\\1physics.py} \\
\texttt{projects/project/\\1a/\\1self/\\1improvement/code/active/\\1inference.py} & \texttt{F:/TMLR/Fusion/enwi/\\1prototype/active/\\1inference.py} \\
\texttt{projects/project/\\1e/\\1verification/code/differentiable/\\1logic.py} & \texttt{F:/TMLR/Fusion/enwi/\\1prototype/differentiable/\\1logic.py} \\
\texttt{projects/project/\\1c/\\1causal/\\1world/code/slot/\\1attention.py} & \texttt{F:/TMLR/Fusion/enwi/\\1prototype/slot/\\1attention.py} \\
...
\begin{itemize}
  \item \texttt{active/\\1inference.py} (7641 B): ENWI Active Inference Engine port.
  \item \texttt{aie/\\1lunarlander.py} (5197 B): AIE training on LunarLander.
  \item \texttt{calibration.py} (4405 B): Monitor calibration (Platt scaling).
  \item \texttt{classic/\\1phase2.py} (8428 B): classic TTC loop.
  \item \texttt{encoders.py} (1104 B): shared encoder utilities.
  \item \texttt{env/\\1state/\\1cloner.py} (4454 B): clone env state for batch eval.
  ...
  \item \texttt{full/\\1integration/\\1v2.py} (20230 B): 4-layer orchestrator v2 with 5-seed sweep.
  \item \texttt{joint/\\1phase2.py} (9744 B): joint Monitor+policy training.
  \item \texttt{lunarlander/\\1phase2.py} (7688 B): LunarLander-specific Phase 2.
  ...
  \item \texttt{orchestrator/\\1q.py} (11600 B): Q-BoN orchestrator.
  \item \texttt{phase26/\\1verifier/\\1gating.py} (13871 B): Phase 2.6 verifier-aware gating.
  \item \texttt{phase27/\\1multiseed.py} (11103 B): Phase 2.7 multi-seed TTC.
  \item \texttt{phase27/\\1threshold/\\1sweep.py} (12155 B): Phase 2.7 threshold sweep.
  \item \texttt{ppo.py} (7951 B): PPO implementation.
  \item \texttt{procgen/\\1baseline.py} (6433 B): Procgen baseline (deferred).
  \item \texttt{procgen/\\1phase2.py} (8341 B): Procgen Phase 2 (deferred).
  \item \texttt{q/\\1bon.py} (11570 B): Q-function with Best-of-N selection.
  \item \texttt{slot/\\1monitor.py} (10418 B): Slot-attention Monitor.
  \item \texttt{ttc/\\1bon/\\1monitor.py} (12147 B): TTC BoN+Monitor loop.
\end{itemize}
...
\begin{itemize}
  \item \texttt{composable/\\1physics.py} (11835 B): ENWI composable physics port.
  \item \texttt{enwi/\\1prediction2.py} (~5000 B): ENWI Prediction 2 replication.
  \item \texttt{slot/\\1attention.py}: slot attention module.
  \item \texttt{slot/\\1dynamics.py}: per-slot dynamics.
\end{itemize}
...
\begin{itemize}
  \item \texttt{language/\\1interface.py} (4065 B): template-based generation.
\end{itemize}
...
\begin{itemize}
  \item \texttt{differentiable/\\1logic.py} (7407 B): ENWI DLR port.
  \item \texttt{dlr/\\1lunarlander.py} (5334 B): DLR LunarLander integration.
  \item \texttt{ltl/\\1verifier.py} (6000 B): LTL verifier.
\end{itemize}
...
\texttt{projects/*/code/checkpoints/} and \texttt{experiments/\\1log/}. The checkpoints directory
...
\begin{itemize}
  ...
  \item \texttt{train/\\1mean}: mean training return.
  \item \texttt{eval/\\1mean}: mean evaluation return.
  \item \texttt{extra}: dict of mode-specific fields (e.g., \texttt{frozen/\\1aurocs}, \texttt{gate/\\1threshold}).
\end{itemize}
...
\texttt{projects/project/\\1a/\\1self/\\1improvement/code/aie/\\1train/\\1full.py}:
...
\begin{itemize}
  \item \texttt{projects/project/\\1a/\\1self/\\1improvement/code/aie/\\1train/\\1full.py} (new, ~7K bytes)
  \item \texttt{projects/project/\\1a/\\1self/\\1improvement/code/checkpoints/aie/\\1full/seed{0,1,2}/phase2\_log.json}
  \item \texttt{experiments/\\1log/2026-07-27-aie-train-full.md}
...
\texttt{projects/project/\\1e/\\1verification/code/dlr/\\1train/\\1full.py}:
...
The DLR verifier (Brier 0.582 on \texttt{F (leg/\\1l AND leg/\\1r)}) is \textit{worse} than LTL
...
\begin{itemize}
  \item \texttt{projects/project/\\1e/\\1verification/code/dlr/\\1train/\\1full.py} (new, ~13K bytes)
  \item \texttt{projects/project/\\1e/\\1verification/code/checkpoints/dlr/\\1full/seed{0,1,2}/phase2\_log.json}
  \item \texttt{experiments/\\1log/2026-07-27-dlr-train-full.md}
...
The original DLR pipeline (\texttt{dlr/\\1train/\\1full.py}) had a critical failure:
...
\texttt{projects/project/\\1e/\\1verification/code/dlr/\\1attention.py}:

\begin{itemize}
  \item \textbf{Learned obs --?slots projection} (\texttt{ObsToSlots}): a small MLP that maps
\end{itemize}

  8-dim observation to \texttt{(n/\\1slots, slot/\\1dim)}. Initialized to give each
...
\begin{itemize}
  \item \texttt{projects/project/\\1e/\\1verification/code/dlr/\\1attention.py} (~270 lines)
  \item \texttt{experiments/\\1log/2026-07-27-dlr-attention.md} (formal log)
  \item \texttt{checkpoints/dlr/\\1attention/seed{0,1,2}/phase2\_log.json}
...
\texttt{projects/project/\\1a/\\1self/\\1improvement/code/full/\\1integration/\\1v3.py}:
...
Twitter / Discord drafts have been prepared (in \texttt{community/twitter/\\1v0p4/\\1halt.md}
and \texttt{community/discord/\\1v0p4/\\1halt.md}) to announce the HALT decision publicly
...
\begin{itemize}
  \item \texttt{code/full/\\1integration/\\1v2.py} (with --imitation, --n-train-episodes flags)
  \item \texttt{experiments/\\1log/2026-07-27-phase15-v0p4-abc.md}
  \item \texttt{experiments/\\1log/phase15/\\16way/\\1summary.json}
  \item \texttt{community/twitter/\\1v0p4/\\1halt.md}
  \item \texttt{community/discord/\\1v0p4/\\1halt.md}
\end{itemize}
...
1. Try \texttt{monitor/\\1lambda} in {1.0, 2.0, 5.0} to find optimal regularizer strength.
...
\begin{itemize}
  \item \texttt{code/y13/\\1monitor/\\1regularizer.py} (~325 lines, by concurrent session)
  \item \texttt{code/ppo/\\1only/\\1baseline.py} (~80 lines)
  \item \texttt{experiments/\\1log/2026-07-27-phase15-y13-monitor-regularizer.md}
  \item \texttt{paper/\\1v2/\\1full.md} Sections 4.10.12-4.10.14
\end{itemize}
...
\begin{itemize}
  \item \texttt{projects/project/\\1a/\\1self/\\1improvement/code/h1/\\1cross/\\1env.py} (~270 lines)
  \item \texttt{projects/project/\\1a/\\1self/\\1improvement/code/h1/\\1cross/\\1env/\\1v2.py} (~290 lines)
  \item \texttt{projects/project/\\1e/\\1verification/code/dlr/\\1cross/\\1env.py} (~190 lines)
  \item \texttt{experiments/\\1log/2026-07-27-h1-cartpole-preliminary.md}
  \item \texttt{experiments/\\1log/2026-07-27-dlr-cross-env-cartpole.md}
\end{itemize}
...
\begin{itemize}
  \item \texttt{experiments/\\1log/2026-07-27-phase15-y13-extend.md}
  \item \texttt{experiments/\\1log/y13/\\1extend/\\1summary.json}
  \item \texttt{projects/project/\\1a/\\1self/\\1improvement/paper/\\1v2/\\1full.md} Sections 4.10.12-4.10.16
\end{itemize}
...
\begin{itemize}
  ...
  \item File: \texttt{papers/y1/\\1paper/\\1draft.md} (~28 KB, 14+ pages with appendices)
  \item Figures: 4 (PNG, reproducible via \texttt{papers/make/\\1figures.py})
  \item Tables: 2 (LaTeX)
\end{itemize}
...
\begin{itemize}
  \item \texttt{papers/y1/\\1paper/\\1draft.md} (~28 KB, full --1-7 + 4 Appendices + 15 References)
  \item \texttt{papers/y1/\\1paper/\\1outline.md} (8.8 KB planning doc)
  \item \texttt{papers/make/\\1figures.py} (reproducible figure generation)
  \item \texttt{papers/y1/\\1fig1-4/\\1*.png} (4 figures)
  \item \texttt{papers/y1/\\1table1-2/\\1*.tex} (2 LaTeX tables)
\end{itemize}
...
\begin{itemize}
  \item We have drafts ready (\texttt{grant/\\1applications/})
...
\begin{itemize}
  \item \texttt{papers/phase2/\\1paper/\\1outline.md} (~10 KB, full --1-9 outline)
  \item \texttt{papers/y1/\\1paper/\\1draft.md} (Y1 paper, single-agent foundation)
