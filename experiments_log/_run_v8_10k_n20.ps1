...
$DONE   = Join-Path $LOGDIR ("_v8_10k_n20_" + $TS + ".done")
"" | Out-File -FilePath $DONE -Encoding utf8
"=== v8 10K-episode n=20 (dlr_only + no_verifier) start $TS ===" | Out-File -FilePath $DONE -Append -Encoding utf8
"=== 40 jobs: 20 dlr_only + 20 no_verifier (s0-s19), 4-parallel ===" | Out-File -FilePath $DONE -Append -Encoding utf8
"=== Goal: confirm dlr_only effect reversal at 10K with more statistical power ===" | Out-File -FilePath $DONE -Append -Encoding utf8
...
while ($launched -lt $jobs.Count) {
    while (($running.Count -lt $MAX_PARALLEL) -and ($launched -lt $jobs.Count)) {
        ...
        $arm = $job.arm
        $s = $job.seed
        $seedLog = Join-Path $LOGDIR ("_v8_10k_n20_" + $arm + "_s" + $s + ".log")
