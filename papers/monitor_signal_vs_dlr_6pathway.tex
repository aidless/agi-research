...
\begin{itemize}
  \item \textbf{Single environment}: we test only on PettingZoo
        ...
        multi-agent).
  \item \textbf{Compute scale (800ep validated at $n=100$, 10K
        tested at $n=20$)}: we use 800 env episodes per seed (80
        PPO updates $\times$ 10 episodes) for the main results.
        Real-world MARL often uses 100K+ episodes. The 10K-
        episode test ($n=5$ then $n=20$) showed the dlr\_only
        effect is \textbf{NOT ROBUST} at longer training. At
        800ep, dlr\_only gives +0.06 over no\_verifier at
        $n=100$ (sig, $p_{\text{bonf}}=0.043$, 95\% CI
        [+0.0084, +0.1149]). At 10K, the $n=5$ result showed
        a direction REVERSAL ($-1.69$, NOT sig), but the $n=20$
        result showed the direction back to positive (+1.27,
        NOT sig at $p=0.15$, 95\% CI [-0.4641, +3.0118]
        INCLUDES 0). The 10K result is confounded by overall
        training instability: no\_verifier at 10K = $-74.02$
        vs $-69.66$ at 800ep, suggesting MADDPG v2 overfits/
        diffuses at 10K with these hyperparameters. The variance
        at 10K is also 14$\times$ higher than at 800ep (sd\_diffs
        = 3.97 vs 0.27). We do NOT recommend dlr\_only for use
        with longer training regimes (10K+ episodes) without
        further hyperparameter tuning. The 800ep result is the
        most reliable estimate of the dlr\_only effect at our
        compute scale.
  ...
  \item \textbf{DLR coefficients not tuned}: we use the default
        DLR coefficients from the Y1 Project E framework. A
        hyperparameter sweep over DLR coefficients might reveal
        larger effects.
\end{itemize}
