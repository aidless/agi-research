...
\begin{itemize}
  \item \textbf{Single environment}: we test only on PettingZoo
        ...
        multi-agent).
  \item \textbf{Single compute scale (800 episodes only validated
        at $n=100$)}: we use 800 env episodes per seed (80 PPO
        updates $\times$ 10 episodes) for the main results.
        Real-world MARL often uses 100K+ episodes. A preliminary
        10K-episode test ($n=5$) showed the dlr\_only effect
        \textbf{REVERSES} at longer training: at 800ep, dlr\_only
        is slightly better than no\_verifier (+0.15 at $n=5$,
        +0.06 at $n=100$); at 10K, dlr\_only is WORSE than
        no\_verifier by $-1.69$ (NOT sig at $n=5$). The 10K
        result is confounded by overall training instability
        (no\_verifier at 10K = $-71.45$ vs $-70.50$ at 800ep,
        suggesting MADDPG v2 overfits/diffuses at 10K with these
        hyperparameters). We do NOT recommend dlr\_only for use
