# The Archimedes Project 閳?A 5-Year Independent Research Program Toward a Self-Improving AGI Substrate

**A Doctoral-Level Thesis Draft, Version 1.0**

---

**Author**: 閸掓ɑ杈伴弬?(Liu Zewen)
**Affiliation**: Independent Researcher
**Date**: 2026-07-27
**Project**: Archimedes (AGI-2026-001)
**Repository**: https://github.com/aidless/agi-research
**License**: MIT 閳?see LICENSE for full attribution requirements
...
a self-improving AGI substrate. Our central hypothesis is that decoupling閳ユ敃eparating
the failure-prediction Monitor from the policy gradient that shapes behavior閳ユ攰s the
...
1. **(A) Decoupled failure-prediction Monitor** 閳?predicts episode failure from history.
2. **(C) Slot-attention world model with per-slot dynamics** 閳?decomposes trajectories
...
3. **(D) Template-based language-as-type-system interface** 閳?converts
   `(Monitor probability, slot states, current observation)` into typed natural
   language descriptions.
4. **(E) Neuro-symbolic verifier** 閳?evaluates fuzzy propositional logic over slot
...
configuration achieves **AUROC 0.989** versus raw-history Monitor 0.796 閳?a 24%
...
epochs of training the composable variant remains **1.9鑴?worse** than the monolithic
baseline.

The thesis concludes with a research roadmap spanning Years 1閳?, identifying the
...
- **Part III 閳?Project C: Causal World Models with Slot Attention**
  - Chapter 10: Slot Attention Background
  - Chapter 11: Per-Slot Dynamics
  - Chapter 12: Composable Physics Port
- **Part IV 閳?Project D: Language Interface**
  - Chapter 13: Template-Based Generation
  - Chapter 14: Type Lattice
- **Part V 閳?Project E: Neuro-Symbolic Verification**
  - Chapter 15: LTL Verifier
  - Chapter 16: Differentiable Logic Reasoner
  - Chapter 17: Verifier-Aware Gating
- **Part VI 閳?Project F: Multi-Agent (Sketch)**
  - Chapter 18: Decentralized Monitor Coordination
- **Part VII 閳?Cross-Environment & Transfer**
  - Chapter 19: LunarLander 閳?CartPole 閳?MountainCar 閳?Procgen
- **Part VIII 閳?Discussion and Future Work**
...
- **Path 3** captures the structural prior that perception is decomposable into
  objects, agents, and relations 閳?necessary for compositional generalization.
...
**Kimi K3 (Moonshot AI, 2026)** 鈥?a 2.8T-parameter open-weight MoE model
...
- KDA's channel-wise forget gate is conceptually similar to our
  **decoupled Monitor**'s policy/freeze decision 鈥?both involve a
...
- **Layer 0 閳?Embodied interface**: a body that produces observations and accepts
  actions.
- **Layer 1 閳?SSM backbone**: a Mamba-class state-space model that compresses
  observation streams into a fixed-rate latent stream.
- **Layer 2 閳?Multi-modal encoders**: perception modules (vision, audio, proprio).
- **Layer 3 閳?Composable physics**: a set of specialized differentiable simulators
  (gravity, collision, friction, inertia) plus a *Composer* that learns to weight
  per-state-action module contributions.
- **Layer 4 閳?Differentiable logic reasoner**: a fuzzy logic engine that evaluates
  quantified logical formulas over slot representations.
- **Layer 5 閳?Active inference engine**: an action-selection module that minimizes
...
grounding; theorems 5閳? decompose free energy; theorems 7閳? establish conditions for
composable physics to outperform a monolithic baseline; theorems 10閳?1 bound the
sample complexity of active inference.

#### Theorem 1 閳?Differentiable Logic Soundness

> For any predicate `锠乣 defined as a product-t-norm combination of base predicates,
> the value `[[锠乚] 閳?[0, 1]` produced by the Differentiable Logic Reasoner
> satisfies `lim_{钄氶埆?} [[锠乚] 閳?1` if and only if 锠?is provably true in classical
> logic.

#### Theorem 2 閳?Differentiable Logic Completeness

> For any classical tautology `锠乣, there exists a finite product-t-norm expression
> `锠刞 such that `[[锠刔] 閳?1 閳?钄歚 for any `钄?> 0`.

#### Theorem 3 閳?Classical Limit
...
#### Theorem 4 閳?JEPA-Symbol Equivalence

> For any observation `o` and any symbolic state `s` in the JEPA representation,
> there exists a predicate `锠乣 such that `[[锠?o)]] = 1` if and only if `s` is a
> faithful abstraction of `o`.

#### Theorem 5 閳?Free Energy Decomposition

> The variational free energy `F` admits the decomposition
> `F = D_KL(q(s) || p(s|o)) 閳?log p(o)`.

#### Theorem 6 閳?Expected Free Energy Decomposition

> For a candidate action `a`, the expected free energy is
> `G(a) = E_{q(s|o,a)}[閳姡og p(o|C) 閳?KL(q(s|o,a) || q(s|o))]`,
> where the first term is pragmatic value and the second is epistemic value.

#### Theorem 7 閳?Composable Physics Sample Complexity
...
> `O(|S|^2 log(1/鏈?)` where `S = 閳?S_i`.

#### Theorem 8 閳?Composable Physics Generalization

> If `M_1, 閳? M_K` cover all reachable scenes with coverage `浼?> 0`, then the
> Composer generalizes to held-out scenes with expected error `O((1 閳?浼?^K)`.

#### Theorem 9 閳?Monolith Lower Bound

> The best monolithic world model of parameter count `P` achieves worst-case error
> at least `鎯?P^{閳?/d})` on `d`-dimensional scenes, while a `K`-module Composer
> with `P/K` parameters per module achieves worst-case error `O((P/K)^{閳?/d} / 浼?`.

#### Theorem 10 閳?Active Inference Data Efficiency

> An active inference agent achieves the same expected return as a PPO baseline
> with `O(T log(1/钄?)` samples where `T` is horizon, beating PPO''s `O(T^2 / 钄?`
> sample complexity for `钄?< 1/T`.

#### Theorem 11 閳?Active Inference Convergence

> Under regularity conditions, the expected free energy `G_t` decreases
> monotonically with iteration `t`, and `lim_{t閳巻鍨秨 G_t = G*`.
...
`(S, A, P, r, 绾?`. In LunarLander-v3 specifically:

- `S 閳?R^8`: continuous state vector (position, velocity, angle, angular velocity,
...
- `r(s, a, s'')`: shaped reward (閳?.3 per timestep + 100 per leg contact + 200 for
  landing 閳?100 for crash).
- `绾?= 0.99`: discount factor.

An episode terminates on crash, landing, or timeout (1000 steps).

### 5.2 The Monitor

The Monitor `M_鑳?: R^d 閳?[0, 1]` is a function approximator parameterized by `鑳僠
that maps an episode history `h_t = (s_0, a_0, r_0, 閳? s_t)` to a predicted
probability that the episode will end in failure. Failure is defined as
`terminal state is crash`, which corresponds to reward `< 閳?00` at termination.
...
> **H1**: A Monitor trained on rollouts from a frozen policy `锜篲f` has higher
> failure-prediction AUROC than a Monitor trained jointly with the policy being
> improved, on the same PPO budget.

**Operationalization**:

- Frozen Monitor: PPO trains a policy `锜篲f` for 100K steps; the policy is then
  frozen; 5000 additional episodes are collected from `锜篲f`; the Monitor is
...
sigmoid output. Input is the flat history vector `h_t 閳?R^200` (50 timesteps 鑴?4 features: `(x, y, vx, vy)` flattened, with actions omitted).
...
```
1. Train PPO for 100K steps 閳?锜篲f
2. Freeze 锜篲f.parameters()
3. Collect 5000 episodes from 锜篲f (deterministic eval)
4. Build Monitor training set: (h_t, label) for t in episode
5. Train Monitor for 50 epochs, batch size 256, Adam lr=1e-3
6. Evaluate on held-out 1000 episodes from 锜篲f
```
...
conclusive at `浼?= 0.05`; a sixth seed would suffice for significance at the
two-sided level.

#### 6.2.3 Why decoupling helps

The frozen Monitor is trained on a stationary distribution of trajectories
(those from `锜篲f`). The joint Monitor is trained on a non-stationary
distribution (those from `锜篲t`, which changes every PPO update). This covariate
...
2. *Pseudo-failures* induced by the policy''s exploration: actions that are bad for
   the current policy but would have been good for `锜篲f`.
...
```
input: h 閳?R^{T 鑴?F}            # T timesteps, F features per step
slots: s 閳?R^{K 鑴?D}            # K slots, D dimensions per slot
...
|-------|--------|--------|----------|
| 2.1 | Naive gating (always action 0) | 閳?92.3 | - |
| 2.2 | Random gating | - | - |
...
### 8.4 DEC-0011 閳?Phase 1.5 5-seed sweep

DEC-0011 documents a 5-seed sweep of the full Phase 1.5 integration (A+C+D+E+Q):

- Mean delta: **+21.5 鍗?67.1** (n=5, sample standard deviation).
...
predictive uncertainty in RL: knowing that an action is risky 閳?knowing which
...
```
input: x 閳?R^{T 鑴?F}            # T timesteps, F features per step
slots: s 閳?R^{K 鑴?D}            # K=4 slots (horizontal, rotation, vertical, residual)
output: per-slot trajectory features
```

The rest of the algorithm is unchanged. Empirically, the 1-D adaptation
converges in 1閳? iterations rather than 3, because the temporal signal is
...
f_k = MLP(D + |A|, 128) 閳?ReLU 閳?MLP(128, 64) 閳?ReLU 閳?MLP(64, D)
```
...
```
loss = mean_k ||f_k(slot_k, a) 閳?slot'_k||^2
```
...
- **Gravity**: `y'' = g 閳?drag * y'`. Implemented as a 2-layer MLP over `(y, v_y)`.
- **Collision**: `v' = 閳姀 * v + (1 + e) * m_other / (m_self + m_other) * v_other`.
  Implemented as an MLP over `(v_self, v_other, m_self, m_other)`.
- **Friction**: `v' = v 閳?娓?* g * sign(v)`. Implemented as an MLP over `(v, 娓?`.
...
### 12.5 Results 閳?honest replication

#### 12.5.1 Smoke test (30 epochs, latent=32)

| scene | monolithic MSE | composable MSE | ratio |
|-------|----------------|----------------|-------|
| free_fall | 2.09e-7 | 2.70e-6 | 10鑴?worse |
| collision | 1.21e-6 | 3.70e-6 | 3鑴?worse |
| friction | 6.69e-7 | 1.57e-6 | 2.5鑴?worse |
| inertia | 2.25e-7 | 5.22e-7 | 2.3鑴?worse |
| compound | 4.64e-7 | 1.25e-6 | 2.7鑴?worse |
| **mean** | **5.55e-7** | **1.95e-6** | **3.5鑴?worse** |
...
remains **1.9鑴?worse** than the monolithic baseline.

### 12.6 Why the negative result

We identify four possible explanations:

1. **Insufficient training**: ENWI used 2000 epochs; we used 100 (20鑴?less).
2. **Synthetic data too simple**: Our scene generator uses linear perturbations;
   ENWI''s uses closed-form physics (gravity t铏? momentum conservation).
3. **Insufficient data**: ENWI used 250K scenes/epoch; we used 1000 (250鑴?less).
...
The LunarLander state space `S 閳?R^8` admits a *type lattice* that decomposes it
into orthogonal subspaces:

```
S = Position 鑴?Velocity 鑴?Rotation 鑴?Contact
```

- **Position**: `(x, y) 閳?R铏廯. Describes where the lander is.
- **Velocity**: `(vx, vy) 閳?R铏廯. Describes how the lander moves.
- **Rotation**: `(angle, ang_vel) 閳?R 鑴?R`. Describes orientation.
- **Contact**: `(leg_l, leg_r) 閳?{0, 1}铏廯. Describes ground contact.

Each type has a domain (R, R, R铏? {0,1}铏? and a *meaning* (where, motion, pose,
touch).
...
- `Position + Velocity * 铻杢 閳?Position`: kinematic integration.
- `Velocity 閳?Velocity`: thrust application.
- `Contact 閳?{landed, crashed}`: terminal state derivation.
...
- `G 锠乣: *globally* 閳?锠?holds at every future step.
- `F 锠乣: *eventually* 閳?锠?holds at some future step.
- `X 锠乣: *next* 閳?锠?holds at the next step.
- `锠?U 锠刞: 锠?holds until 锠?holds.

Our LTL verifier supports a subset: `G`, `F`, `X`, `U`, plus propositional
operators `閳湨, `閳暅, `鍗, `閳妶.
...
- `velocity_below(state, threshold)`: 閳ユ潳閳?< threshold.
- `angle_below(state, threshold)`: |angle| < threshold.
- `landed(state)`: terminal state with reward 閳?100.
- `in_pad(state)`: position within landing pad.
- `distance_to_pad(state)`: 閳ユ潷 閳?x_pad閳?
...
2. Builds a B鐪塩hi automaton from the LTL formula.
...
in the limit `a, b 閳?{0, 1}`.

### 16.3 Quantifiers

Universal and existential quantifiers over a set of values:

- `FORALL(values) = 閳?values`
- `EXISTS(values) = 1 閳?閳?1 閳?values)`

These reduce to classical `閳偓` and `閳儬 in the crisp limit.
...
- `landed`: terminal state with reward 閳?100.
- `upright`: |angle| < 0.3.
...
- `monitor_prob` (1-dim)
- `slot_states` (4 鑴?32-dim = 128-dim)
...
(1 per timestep alive), and a single failure mode (pole falls > 15鎺?.
...
- Continuous action space (no 閳?discrete, 4 actions).
- Partial observability (no 閳?fully observable).
...
### 21.2 Composable physics vs monolithic (1.9鑴?worse at 100 epochs)
...
rejected), the joint Monitor is essentially useless (AUROC 閳?0.07, near
...
The Phase 1.5 5-seed sweep produced delta = +21.5 鍗?67.1, which is not
statistically significant. The Monitor signal is real but the *online gating
signal* is too noisy. More seeds (n 閳?10) or better calibration are needed.
...
The central architectural lesson 閳?*decoupling is the core mechanism for stable
self-monitoring* 閳?is both robust and actionable. We expect this insight to
...
## Appendix A: Eleven ENWI Theorems 閳?Detailed Proofs

### A.1 Theorem 1 閳?Differentiable Logic Soundness (sketch)

**Statement**: For any predicate `锠乣 defined as a product-t-norm combination of
base predicates, the value `[[锠乚] 閳?[0, 1]` produced by the DLR satisfies
`lim_{钄氶埆?} [[锠乚] 閳?1` iff 锠?is provably true in classical logic.

**Proof sketch**: The product t-norm is *sound* with respect to classical
propositional logic: in the crisp limit (`a, b 閳?{0, 1}`), the operations
...
Formally: for any formula `锠?x)` over base predicates `p_i`, let `[[锠乚]_鑳僠 be the
value computed by the DLR with parameters `鑳僠. As `鑳?閳?鑳?` (the limit of perfect
predicate approximation), we have `[[锠乚]_鑳?閳?[[锠乚]_classical 閳?{0, 1}`.
...
### A.2 Theorem 4 閳?JEPA-Symbol Equivalence

**Statement**: For any observation `o` and any symbolic state `s` in the JEPA
representation, there exists a predicate `锠乣 such that `[[锠?o)]] = 1` iff `s`
is a faithful abstraction of `o`.

**Proof sketch**: The JEPA encoder `E: O 閳?S` maps observations to symbolic
states. A faithful abstraction means `s` preserves all *task-relevant* features
of `o`. The predicate `锠乣 is constructed as:

```
锠?o) := AND_k (p_k(E(o)) 閳?p_k(s))
```

where `p_k` ranges over all task-relevant predicates. By construction,
`[[锠?o)]] = 1` iff `s` agrees with `E(o)` on every `p_k`, which is the
definition of faithful abstraction.

### A.3 Theorem 7 閳?Composable Physics Sample Complexity
...
sample complexity `O(|S|^2 log(1/鏈?)` where `S = 閳?S_i`.
...
`O(sqrt(log(1/鏈? / n))`. Setting the bound to `钄歚 gives
`n = O(log(1/鏈? / 钄氳檹)`. For 钄?= 1/|S|, we have `n = O(|S|铏?log(1/鏈?)`.
...
### A.4 Theorem 10 閳?Active Inference Data Efficiency

**Statement**: An active inference agent achieves the same expected return as
a PPO baseline with `O(T log(1/钄?)` samples where `T` is horizon, beating PPO''s
`O(T铏?/ 钄?` sample complexity for `钄?< 1/T`.
...
sample complexity `O(log(1/钄?)` for return accuracy `钄歚.

PPO uses policy gradient with high variance; the variance is `O(T)` (one
sample per trajectory), giving sample complexity `O(T铏?钄?`.

The crossover is at `钄?= 1/T`: active inference is more efficient for tight
...
### B.1 Initial skeleton (commits 1閳?0)
...
### B.2 Project A: TTC + Monitor (commits 11閳?0)
...
- `1093b82` Phase 1.2: Slot World Model 閳?next-step error 0.000007
- `df0ef81` Phase 1.5: Full 4-Layer AGI Integration (A+C+D+E+Q) 閳?WORKING
- `4957cd4` v0.2.0: Phase 1.5 full 4-layer AGI integration - 100K PPO results
- `c24d5cc` Phase 2.6: Verifier-Aware Gating 閳?architecture works, calibration issue
- `6690c55` Phase 2.7: 100K PPO honest eval (gating -61.8, vs -467 at 4K)
- `e4c836f` Phase 2.7: Gate threshold sweep 閳?thresh=0.6 is sweet spot (+27 over ungated)
- `e59710e` Phase 2.7 multi-seed: HONEST negative 閳?gating doesn''t help
- `a83c247` DEC-0011: Phase 1.5 5-seed sweep + Phase 2.7 cross-validation

### B.3 ENWI port (commits 31閳?0)

- `12fe5f2` ENWI Prediction 2 port (composable physics) 閳?smoke NEGATIVE
...
- `e80e768` ENWI Prediction 2 閳?100-epoch: composable 1.9鑴?WORSE than monolithic
...
| ENWI theorems | `F:\TMLR\Fusion\ENWI_PAPER.md` 鎼?閳ユ拃? |
| ENWI predictions | `F:\TMLR\Fusion\ENWI_PAPER.md` 鎼? |
...
### Project C 閳?Causal World Model
...
### Project D 閳?Language Interface

- `language_interface.py` (4065 B): template-based generation.

### Project E 閳?Verification
...
This thesis is the sole work of the author, 閸掓ɑ杈伴弬?(Liu Zewen), with AI
...
[2] Friston, K. (2010). The free-energy principle: a unified brain theory?
    *Nature Reviews Neuroscience*, 11(2), 127閳?38.

[3] LeCun, Y. (2022). A Path Towards Autonomous Machine Intelligence.
    *OpenReview preprint*.

[4] Sch鏋歭kopf, B., Locatello, F., Bauer, S., Ke, N. R., Kalchbrenner, N.,
    Goyal, A., & Bengio, Y. (2021). Toward Causal Representation Learning.
    *Proceedings of the IEEE*, 109(5), 612閳?34.
...
[38] Mnih, V., Kavukcuoglu, K., Silver, D., Rusu, A. A., Veness, J., Bellemare,
    ...
    Reinforcement Learning. *Nature*, 518, 529閳?33.
...
[46] Moonshot AI. (2026). Kimi K3: Open Frontier Intelligence 閳?Technical
...
[47] Moonshot AI. (2026). FlashKDA: Flash Kimi Delta Attention 閳?High-
...
that will be updated quarterly as the Y1閳ユ彠5 program progresses.
...
*Thesis draft v1.0, generated 2026-07-27, by 閸掓ɑ杈伴弬?(Liu Zewen) with Codex
assistance. 1673 lines, ~85 KB Markdown. Target: 100+ pages when rendered.*

*"Eureka! Eureka!" 閳?Archimedes*


---

# Addendum (2026-07-27 evening) 閳?AIE Training & DLR Integration
...
- 8 outer iterations 鑴?8 episodes per outer = ~64 episodes per seed.
...
This is the expected outcome: AIE requires 10鑴?more compute than we have to
...
The AIE loss decreases monotonically (21.7 閳?19.5), so the model is
...
- Run AIE at 100K steps (10鑴?current budget) for a fair PPO comparison.
...
- Compute: ~42 sec per seed on CPU (8 outer 鑴?8 episodes 鑴?~150 steps/episode).
...
The thesis Chapter 15閳?7 documents the LTL verifier and the DLR port (smoke
...
4. **Compositional**: 閳偓 and 閳?quantifiers over slot representations.
...
### B.3 Results 閳?Predicate Accuracy
...
### B.4 Verification Comparison 閳?DLR vs LTL
...
- For *continuous conditional* formulas (G landed 閳?in_pad), both struggle
...
## Addendum Chapter C: Summary 閳?Three Engineering Outcomes

This evening's session produced three engineering outcomes:

| Outcome | Status | Honest assessment |
|---------|--------|--------------------|
| Thesis expansion v1.0 | 閴?2050 lines, 77.7 KB, 8 parts + appendices | Significant growth from v0.1 (313 lines, 10.6 KB) |
| AIE full training | 閴?3 seeds, loss decreases 21.7 閳?19.5 | Modest learning (-139 mean); not competitive with PPO |
| DLR full training | 閴?3 seeds, 7 predicates trained | 94% mean accuracy on 6/7 predicates; `upright` fails |
...
# Addendum (2026-07-27 late) 閳?DLR Attention Fix & DEC-0011 v0.3 Attempt
...
**45% accuracy** 閳?essentially random. The root cause was:
...
- **Learned obs 閳?slots projection** (`ObsToSlots`): a small MLP that maps
...
The `upright` problem is fixed (45% 閳?89%). All 7 predicates now exceed
...
After v0.2 (REJECTED 閳?strong negative), we designed v0.3 with three fixes
based on the failure analysis:

1. **Skip Platt scaling** 閳?the val_auroc=1.0 was overfit to a tiny val set;
   Platt just amplified the overfit, collapsing cal_threshold to ~0.
2. **Use a FIXED high threshold (0.7)** 閳?instead of calibrated threshold.
3. **Larger val set (200 episodes)** 閳?to reduce overfitting risk.
4. **Skip Q entirely** 閳?when Monitor fires, use safe_action=0 (do nothing)
   instead of Q-BoN argmax. Rationale: v0.2 CQL Q (200 train eps) was bad;
   Q-BoN picked bad actions.
5. **Temporal hysteresis** 閳?only gate if Monitor has been high for the
...
[Y0 Q3 closing 閳?preliminary; full 5-seed sweep pending in background]
...
- H1 ablation (5 seeds 鑴?100K PPO): ~2.5 hours wall time.
- Slot-Monitor: ~30 min wall time.
- ENWI Prediction 2 (100 epoch): ~3 min wall time.
- DLR training (3 seeds 鑴?30 epochs): ~2 min wall time.
- DEC-0011 sweep (5 seeds 鑴?full pipeline): ~2.5 hours wall time.

All scripts commit JSON logs to `checkpoints/` for full reproducibility.

### F.4 The DEC-0011 series as a case study

The DEC-0011 series (v0.1 mixed 閳?v0.2 rejected 閳?v0.3 attempt) illustrates
...
## Addendum Chapter G: DEC-0011 v0.4 閳?Six-Way Comprehensive Sweep + HALT Decision
...
### G.3 v0.4A 閳?the ONE positive finding
...
### G.4 v0.4B 閳?different environment fails too
...
### G.5 v0.4C 閳?imitation learning doesn't help either
...
## Addendum Chapter H: Y1.3 閳?Monitor as Training-Time Regularizer (BREAKTHROUGH)

### H.1 The breakthrough

After 6 failed attempts at inference-time gating (v0.1 閳?v0.4C), the
...
- seed 0: **+64.2** (75.6 vs 11.4) 閳?win
- seed 1: -58.7 (29.2 vs 87.9) 閳?loss
- seed 2: **+84.8** (105.2 vs 20.4) 閳?win
- seed 3: **+105.4** (178.7 vs 73.3) 閳?win
- seed 4: **+53.6** (63.8 vs 10.2) 閳?win
...
### I.2 H1 on CartPole-v1 閳?inconclusive (environment saturated)
...
### I.3 DLR on CartPole-v1 閳?STRONG POSITIVE
...
1. Lower-dim state (4 vs 8) 閳?easier projection
...
| **DLR attention** | 閴?Validated on CartPole | Architecture is env-agnostic |
...
## Addendum Chapter J: Y1.3 EXTENDED 閳?First Statistically Significant Result (2026-07-28)
...
|--------------|-------------|--------------|---------|
| LunarLander-v3 (n=15) | **80.1 +/- 45.9** | ~40 baseline | 閴?**POSITIVE** (p<0.001) |
| Acrobot-v1 (n=5) | -88.7 +/- 8.3 | typical -80 to -100 | 閳跨媴绗?NEUTRAL (similar to baseline) |
| MountainCar-v0 (n=5) | -200.0 +/- 0.0 | -200 (PPO doesn't converge) | 閴?PPO doesn't converge; Y1.3 doesn't help |
...
3. **The training-time use of Monitor is the right direction**: Y1.3 confirms
   the conclusion from Addendum H 閳?auxiliary signals are valuable as
...
## Addendum Chapter K: Y1 Paper 閳?Honest Framing Synthesis (2026-07-28)
...
| Pre-registration | **NOT done** 閳?honest gap |
| Peer review | **NOT done** 閳?honest gap |
| Independent replication | **NOT done** 閳?honest gap |
...
The user's feedback "娴犮儱鎮楅柈鍊燁洣鐠囨艾鐤勬稉宥堫洣閼奉亜妫? (always be honest, don't
self-hype) prompted a reframe of how we present results. The change:

| Before | After |
|--------|-------|
| "STRONG POSITIVE" without limits | "STRONG POSITIVE 閳?4-env 97.8% mean, predicates hand-coded, same-distribution test" |
...
- `papers/y1_paper_draft.md` (~28 KB, full 鎼?-7 + 4 Appendices + 15 References)
...
**Hypothesis test**: 5 seeds 鑴?3 envs 鑴?4 methods = 60 runs
...
| 2027-05 | Analysis + draft 鎼?-3 |
| 2027-06 | Draft 鎼?-6 + appendix |
...
- Budget conservatively (5 seeds 鑴?3 envs)
...
- `papers/phase2_paper_outline.md` (~10 KB, full 鎼?-9 outline)
...
---
# ========================================================================
# THESIS DRAFT v1.1 (2026-07-30)
# ========================================================================
# This file is v1.0 + the new chapters for Y3, Y4, Y5 (saved in
# thesis_additions_v1_1.md). To create a fully integrated v1.1, the
# new chapters should be moved to their proper positions in the thesis
# structure (e.g., Chapter 19 replaces/extends the Project F sketch,
# Chapter 20 is a new Part IX, Chapter 21 is a new chapter in Part VIII).
#
# For now, the new chapters are appended at the end of the document
# for easy reference. The integration step is left to the author.
---
...
# Chapter 20: Project G 鈥?LLM Self-Monitoring (Y4, 2026-07-29)
