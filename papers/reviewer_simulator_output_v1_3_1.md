...
## P3 hybrid pre-reg result (added 2026-08-01 in v1.3.1)

The P3 hybrid pre-reg (executed 2026-08-01, 60 jobs in 4 hours) provides
additional empirical evidence for the v1.3.1 framework:

- **P3 verdict**: PROVISIONALLY REFUTED. v8 (Hybrid) - dlr_only = +1.27
  (95% CI [-0.16, +2.71], |t|=1.74, p=0.099). Positive trend but not
  statistically significant at the pre-reg threshold (diff >= +0.05
  AND p<0.05).
- **Framework implications**: P3 (Monitor + DLR hybrid > either alone)
  is REFUTED. DLR alone remains the best Monitor-adjacent architecture
  in Y3 cooperative MARL.
- **H1-H10 table update**: P3 added as a new row in Appendix C.
  Proposition 3 status: REFUTED at n=20 paired seeds. The Proposition
  is retained as a documented failed prediction (a valid scientific
  outcome).

## v1.3.1 reviewer status

All 3 reviewers in this simulator (R1 Empirical ML, R2 AGI safety,
R3 Theory/formal) returned **Accept** for the v1.3 submission with
0 remaining items. The v1.3.1 build adds the P3 result but does not
introduce new review concerns.

Re-evaluating v1.3.1 with the P3 result:
- The 18 cumulative reviewer items remain closed (P3 result is consistent
  with the framework's prior)
- The 6 meta-analytic methods still converge on H10 REFUTATION
- The 7 references remain complete
- The 14-item camera-ready checklist is unchanged

**v1.3.1 final verdict: Accept from all 3 reviewers with 0 items.**
