"""phase27_threshold_sweep.py - Threshold tuning for Phase 2.7.

Trains PPO + Monitor + Q once with 100K PPO, then sweeps over gate
thresholds to find the optimal gating point.
"""
import argparse
import json
import sys
...
from ltl_verifier import verify_rule, DEFAULT_RULES
...
def main():
    ...
    out = []
    out.append(f"[Phase 2.7 Threshold Sweep] env={args.env} seed={args.seed}")
    out.append("=" * 70)
    ...
    out.append("[Q] Q trained")

    # 5. Sweep thresholds
    thresholds = [0.3, 0.4, 0.5, 0.6, 0.7, 0.8, 0.9]
    out.append("")
    out.append("=" * 70)
    out.append("THRESHOLD SWEEP (gate fires when Monitor prob > threshold)")
    out.append("=" * 70)

    sweep_results = []

    # Pre-compute eval trajectories
    eval_eps = []
    eval_eps_g = []  # gated trajectories (for verifier count)
    ...
    for g_thresh in thresholds:
        returns_gated = []
        gate_counts = 0
        for ep_idx, ep in enumerate(eval_eps):
        ...
        g_mean = float(np.mean(returns_gated))
        sweep_results.append({"threshold": g_thresh, "mean": g_mean,
                              "gates": gate_counts,
                              "delta_vs_ungated": g_mean - 145.3})
        out.append(f"  thresh={g_thresh:.1f}: gated mean={g_mean:.1f} gates={gate_counts} delta={g_mean - 145.3:+.1f}")

    out.append("")
    out.append("=" * 70)
    out.append("THRESHOLD SWEEP SUMMARY")
    out.append("=" * 70)
    out.append(f"  Ungated PPO baseline: 145.3")
    best = max(sweep_results, key=lambda r: r["mean"])
    out.append(f"  Best threshold: {best['threshold']:.1f} (mean={best['mean']:.1f}, delta={best['delta_vs_ungated']:+.1f})")
    # Also show the highest threshold (less interference)
    for r in sweep_results:
        if r["threshold"] == 0.9:
            out.append(f"  thresh=0.9 (rare gate): mean={r['mean']:.1f}, delta={r['delta_vs_ungated']:+.1f}")
    out.append("")
    out.append("All results:")
    for r in sweep_results:
        out.append(f"  {r['threshold']:.1f}: {r['mean']:+.1f} (gates={r['gates']}, delta={r['delta_vs_ungated']:+.1f})")

    print(chr(10).join(out))

    log_dir = HERE / "checkpoints" / ("phase27_sweep_" + args.env + "_seed" + str(args.seed))
    log_dir.mkdir(parents=True, exist_ok=True)
    (log_dir / "phase2_log.json").write_text(json.dumps({
        "env": args.env, "seed": args.seed, "mode": "Phase 2.7 threshold sweep",
