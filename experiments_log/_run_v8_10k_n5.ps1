...
$DONE   = Join-Path $LOGDIR ("_v8_10k_n5_" + $TS + ".done")
"" | Out-File -FilePath $DONE -Encoding utf8
"=== v8 10K-episode n=5 (dlr_only + no_verifier) start $TS ===" | Out-File -FilePath $DONE -Append -Encoding utf8
"=== 10 jobs: 5 dlr_only + 5 no_verifier, 800 PPO updates x 10 ep ===" | Out-File -FilePath $DONE -Append -Encoding utf8
"=== Goal: does dlr_only effect grow, shrink, or stay at 10K compute? ===" | Out-File -FilePath $DONE -Append -Encoding utf8
...
while ($launched -lt $jobs.Count) {
    while (($running.Count -lt $MAX_PARALLEL) -and ($launched -lt $jobs.Count)) {
        ...
        $arm = $job.arm
        $s = $job.seed
        $seedLog = Join-Path $LOGDIR ("_v8_10k_" + $arm + "_s" + $s + ".log")
...
"All 10 jobs complete at " + (Get-Date -Format "yyyy-MM-dd HH:mm:ss") | Out-File -FilePath $DONE -Append -Encoding utf8
