"""pz_maddpg_trusthead_same_agent.py - MADDPG trust-head ablation (formerly v5).

Y2 path c (renamed 2026-07-29 after honest review): the third and final
Monitor-in-MA attempt. The original v5 design claimed to use a cross-agent
evidence chain as input to a per-agent trust head. After audit, the trust
head input was found to be DEGENERATE: others_stats was the same-agent
monitor broadcast to (n_agents-1) slots, so the trust head saw only
(my_obs, my_monitor, my_monitor, my_monitor) -- no cross-agent info.
The hash_chain_entry / CHAIN_WINDOW machinery was defined but NEVER READ.

Renamed file to make this honest: this is a 'trust head with same-agent
monitor only' experiment, NOT a cross-agent evidence chain. Findings
(+0.17 with_verifier vs no_verifier, n=5; v7 ablation = v5 = 0.00) are
consistent with the trust-head architecture helping weakly while the
Monitor signal is ignored.
...
Architecture (actual):
  ...
  - Trust head: takes (my_obs, my_monitor_prob) -> per-other-agent trust
    weights in [0,1]. others_stats slot is filled with the same-agent
    monitor broadcast (proxy, NOT real cross-agent info).
  - Actor: trust-weighted sum of OTHER agents Q (plus own Q) in actor loss.
...
def main():
    ...
    p.add_argument("--arm", type=str, default="with_verifier", choices=["with_verifier", "no_verifier", "random_verifier"])
    args = p.parse_args()
    print("=" * 60)
    ...
    log_path.write_text(json.dumps({
        ...
        "mode": "MADDPG trust-head same-agent monitor (formerly v5; cross-agent chain not wired)" if use_verifier else ("MADDPG v2 baseline" if not random_verifier else "MADDPG v5 (random verifier)"),
