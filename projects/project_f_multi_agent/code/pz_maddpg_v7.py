"""pz_maddpg_v5.py - MADDPG v5: Monitor as MA verifier via cross-agent evidence chain.

Y2 path c: the third and final Monitor-in-MA attempt. Unlike v3 (aux loss
in critic) and v4 (inter-agent messages in critic), v5 puts Monitor
signal at the ACTOR level: a trust head uses a per-agent Monitor to
build a cross-agent evidence chain, then gates which other agents the
actor should 'trust' in the centralised critic's Q values.
...
Architecture:
  ...
  - Evidence chain: each step, per-agent monitor_prob -> SHA256 entry
    (timestamp, agent_id, monitor_prob, prev_hash)
  - Trust head: takes (my obs, my Monitor, chain summary) -> per-other-agent
    trust weights in [0,1]
  - Actor: computes its action as in v2, but the actor loss uses
    trust-weighted sum of OTHER agents' Q (not own Q). This is the
    cross-agent credit assignment signal.

Three arms:
  - with_verifier: real Monitor + evidence chain + trust head
  - no_verifier:  MADDPG v2 baseline (no trust head)
  - random_verifier: random Monitor + evidence chain + trust head (control)

Matched compute: 80 updates x 10 episodes = 800 env episodes.
"""
import argparse
import hashlib
import json
...
ACTION_DIM = 5
HISTORY_LEN = 20
CHAIN_WINDOW = 30  # last N evidence entries fed to trust head
...
class TrustHead(nn.Module):
    ...
    chain_summary_features: per-other-agent mean monitor prob over last CHAIN_WINDOW steps.
...
def hash_chain_entry(agent_id, step_idx, monitor_prob, prev_hash):
    """One SHA-256 hash over (agent_id, step_idx, monitor_prob, prev_hash)."""
    payload = (str(agent_id) + "|" + str(step_idx) + "|" + str(round(float(monitor_prob), 6)) + "|" + str(prev_hash)).encode("utf-8")
    return hashlib.sha256(payload).hexdigest()[:16]  # short hash


def collect_episode_v5(actors, monitors, env, seed, noise_scale=0.1,
                        use_verifier=True, random_verifier=False,
                        trusthead_random=False):
    """Collect one episode; build per-agent evidence chain as we go."""
    env.reset(seed=seed)
        ...
        if not done:
            ...
            with torch.no_grad():
                ...
                if use_verifier or random_verifier:
                    ...
                    if random_verifier or trusthead_random:
...
def train_maddpg_v5(seed=0, n_updates=80, n_episodes=10, batch_size=128,
    ...
    # Trust head (only used when use_verifier)
    trust_heads = None
    if use_verifier:
    ...
    for u in range(10):
        ...
        for ep in range(n_episodes):
            transitions, ep_return = collect_episode_v5(
                ...
                trusthead_random=use_trusthead_random)
    ...
    for u in range(n_updates):
        ...
        for i in range(N_AGENTS):
            ...
            if not use_verifier:
                ...
                others_stats = mon_b.unsqueeze(-1).expand(-1, N_AGENTS - 1)  # (B, N_AGENTS-1) proxy
...
def main():
    ...
    p.add_argument("--arm", type=str, default="with_verifier", choices=["with_verifier", "no_verifier", "random_verifier", "with_trusthead_random"])
    args = p.parse_args()
    print("=" * 60)
    print("MADDPG v5 - arm=" + args.arm + " - PettingZoo Simple Spread v3 (continuous)")
    print("=" * 60)
    ...
    print()
    use_verifier = (args.arm in ("with_verifier", "with_trusthead_random"))
    random_verifier = (args.arm == "random_verifier")
    use_trusthead_random = (args.arm == "with_trusthead_random")
    actors, monitors, trust_heads, history, mon_aurocs = train_maddpg_v5(
        ...
        use_verifier=use_verifier, random_verifier=random_verifier,
        use_trusthead_random=use_trusthead_random)
    print()
    ...
    print("  MADDPG v5 (" + args.arm + ") eval: " + str(round(final_mean, 2)) + " +/- " + str(round(final_std, 2)) + "  (delta vs random: " + ("%.2f" % delta) + ")")
    if mon_aurocs:
        print("  Monitor AUROC per agent: " + str([round(a, 3) if a == a else "nan" for a in mon_aurocs]))
    log_path = HERE / "checkpoints" / "pz_maddpg_v5" / ("seed" + str(args.seed) + "_" + args.arm) / "phase2_log.json"
    log_path.parent.mkdir(parents=True, exist_ok=True)
    log_path.write_text(json.dumps({
        "env": "PettingZoo Simple Spread v3 (continuous)",
        "seed": args.seed, "arm": args.arm,
        "mode": "MADDPG v7 (trust head + random inputs)" if use_trusthead_random else ("MADDPG v5 (Monitor as MA verifier via evidence chain + trust head)" if use_verifier else ("MADDPG v2 baseline" if not random_verifier else "MADDPG v5 (random verifier)")),
        ...
        "honest_note": "Y2 path c: Monitor as MA verifier (not critic-side extra). Trust head at actor level.",
    }, indent=2))
